    ...
    def _setup_routes(self):
        ...
        async def list_workflows(
            ...
            created_after: Optional[str] = Query(
                None, description="Filter workflows created after (ISO datetime)"
            ),
            created_before: Optional[str] = Query(
                None, description="Filter workflows created before (ISO datetime)"
            ),
            limit: int = Query(
                ...
                if created_after or created_before:
                    ...
                    if created_after:
                        try:
                            ...
                            date_filter = date_filter.where(
                                first_at_subq.c.first_at >= after_dt
                            )
                    ...
                    if created_before:
                        try:
                            ...
                            date_filter = date_filter.where(
                                first_at_subq.c.first_at <= before_dt
                            )
                        except (ValueError, TypeError):
                            pass
            ...
            async with self.session_maker() as s:
                r1 = await s.execute(
                    ...
                    .where(
                        and_(
                            self.event_model.workflow_id == workflow_id,
                            self.event_model.workflow_version <= v1,
                        )
                    )
                    .order_by(self.event_model.workflow_version.asc())
                )
                r2 = await s.execute(
                    ...
                    .where(
                        and_(
                            self.event_model.workflow_id == workflow_id,
                            self.event_model.workflow_version <= v2,
                        )
                    )
                    .order_by(self.event_model.workflow_version.asc())
                )
                events1 = r1.scalars().all()
                events2 = r2.scalars().all()
                if not events1:
                    raise HTTPException(
                        status_code=404, detail=f"No events for version {v1}"
                    )
                if not events2:
                    raise HTTPException(
                        status_code=404, detail=f"No events for version {v2}"
                    )
                ...
                return {
                    ...
                    "state_v1": {
                        "version": v1,
                        "events": to_dict(events1),
                    },
                    "state_v2": {
                        "version": v2,
                        "events": to_dict(events2),
                    },
                }
        ...
        async def list_events(
            ...
            created_after: Optional[str] = Query(
                None, description="Filter events after (ISO datetime)"
            ),
            created_before: Optional[str] = Query(
                None, description="Filter events before (ISO datetime)"
            ),
            ...
            async with self.session_maker() as s:
                ...
                if created_after:
                    try:
                        ...
                        query = query.where(self.event_model.at >= after_dt)
                    except (ValueError, TypeError):
                        pass
                if created_before:
                    try:
                        ...
                        query = query.where(self.event_model.at <= before_dt)
                    except (ValueError, TypeError):
                        pass
        ...
        @self.app.post("/api/workflows/batch/cancel")
        async def batch_cancel(body: dict = Body(...)):
            """Cancel multiple workflows. Requires Fleuve gateway integration."""
            workflow_ids = body.get("workflow_ids", [])
            if not workflow_ids:
                raise HTTPException(400, detail="workflow_ids required")
            raise HTTPException(
                501,
                detail="Batch cancel requires Fleuve gateway. "
                "Use POST /commands/{workflow_type}/{workflow_id}/cancel per workflow.",
            )

        @self.app.post("/api/workflows/batch/replay")
        async def batch_replay(body: dict = Body(...)):
            """Replay multiple workflows. Requires Fleuve integration."""
            workflow_ids = body.get("workflow_ids", [])
            if not workflow_ids:
                raise HTTPException(400, detail="workflow_ids required")
            raise HTTPException(
                501,
                detail="Batch replay requires Fleuve repo integration. Not yet implemented.",
            )
